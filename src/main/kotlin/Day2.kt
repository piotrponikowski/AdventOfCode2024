class Day2(input: List<String>) {
    ...
    private fun isIncreasing(report: List<Int>) = report.windowed(2).all { (prev, next) -> next - prev in 1..3 }

    private fun isDecreasing(report: List<Int>) = report.windowed(2).all { (prev, next) -> prev - next in 1..3 }

    private fun fixLevels(report: List<Int>) = report.indices
        .map { index -> report.filterIndexed { other, _ -> index != other } }
}